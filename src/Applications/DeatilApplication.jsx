...
useEffect(()=>{
  ...
  const response=await axios.get(`https://internshipbackend-7qgp.onrender.com/api/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://internshipbackend-7qgp.onrender.com/api/application/${id}`,{action})
